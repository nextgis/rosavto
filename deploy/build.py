...
if path.islink(link_to_rosavto):
    ...
    if len(current_release_version) == 1:
        if current_release_version[0][1:] == release_number:
            print 'release {0} already assembled'.format(release_number)
        else:
...
if not path.exists(link_to_ngw_rosavto):
    raise Exception('path "{0}" to NEXTGISWEB_ROSAVTO symbolic link folder not found'.format(link_to_ngw_rosavto))
